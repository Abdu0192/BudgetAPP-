...
public class Budget {
    ...
    public void draw(Graphics g, JButton button, JButton inkomstButton) {
        ...
        public static int  budgettest(int inkomst){

        if (inkomst >=100 && inkomst <1000 ){

            return  (inkomst/100*10);
        }

          else  if (inkomst >=1000 && inkomst <10000){
                return  (inkomst/100*20);
            }

           else if (inkomst >=10000 && inkomst <2147483647){
                return  (inkomst/100*30);
            }
        return 0;
        }


        public static int appcoste(int leeftijd, boolean inkomst, int jaar, boolean advies){

        if (leeftijd < 18 && inkomst &&( advies || (jaar >=0))){return 10;}
       else if (leeftijd < 18 && !inkomst &&( advies || (jaar >=0))){return 5;}

        else if( leeftijd >= 18 && inkomst &&( advies || (jaar >=0) )){ return 10;}
        else if( leeftijd >= 18 &&!inkomst &&( advies || (jaar >=0)) ){ return 5;}
...
public  void printrapport (){
    HashMap<String, Double> size = transactions.getTranacties();

    boolean transcount = !size.isEmpty();
    boolean budget = getBudget()>0;
    int days = 31;
    boolean day = days >31;

      if( Budgetrapportgemaaktals( budget, day, transcount)){
          System.out.println("Rapport succesful gemaakt");
      }
